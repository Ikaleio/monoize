...
Charge = (model charge + tool charge) × Channel model `multiplier` × routed group `billing_ratio`. Check the version 3 billing breakdown in **Logs**. Then check all three inputs on the **Models** and **Providers** pages.

### `403 model_pricing_required`

Add an enabled and complete price on **Models**. Alternatively, allow free settlement globally or on the selected Provider.

### `403 usage_required`

Make the upstream return Usage. Alternatively, allow free settlement for missing Usage globally or on the selected Provider.
