...
6. Enable **Allow missing usage** only when a Channel can omit usage. Monoize then records zero usage and charges zero.
7. Enable **Allow unpriced built-in tools** only when missing tool meter data can receive zero tool charge. Token charges and priced tool charges still apply.
