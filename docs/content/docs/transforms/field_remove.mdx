...
Request paths `reasoning.effort`, `reasoning.summary`, `reasoning.mode`, `reasoning.budget_tokens`, and `reasoning.display` control normalized reasoning settings. Other paths keep their extra-body behavior.

Removing a reasoning setting also removes its native value. Protocol conversion cannot restore it from an earlier request.
