 ...
 */
export function ChannelDialog({ open, onOpenChange, channel, channels }: ChannelDialogProps) {
  ...
  const setType = (type_id: string) => {
    setForm((prev) => ({
      ...prev,
      type_id,
      currency: type_id === "epay" ? "CNY" : prev.currency === "CNY" ? "USD" : prev.currency,
      config: {},
    }));
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!isEdit) {
      ...
      const missingRequired = configFields.some(
        (field) => !field.secret && !field.optional && !(form.config[field.key] ?? "").trim(),
      );
        ...
        <div className="grid gap-4 py-2">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="grid gap-2">
              ...
              <Select value={form.type_id} onValueChange={setType} disabled={isEdit}>
            ...
            <div className="grid gap-2">
              <Label htmlFor="channel-currency">{t("payments.currency")}</Label>
              <Input
                id="channel-currency"
                value={form.currency}
          ...
          <div className="grid gap-2">
            ...
            <Input
              ...
              placeholder="7.30"
            />
            <p className="text-xs text-muted-foreground">{t("payments.usdRateHelp")}</p>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="grid gap-2">
              ...
              <Input
                ...
                onChange={(e) => setForm({ ...form, min_credit_usd: e.target.value })}
              />
            </div>
            <div className="grid gap-2">
              ...
              <Input
                ...
                onChange={(e) => setForm({ ...form, max_credit_usd: e.target.value })}
              />
            </div>
          </div>
          ...
          {configFields.map((field) => (
            <div key={field.key} className="grid gap-2">
              ...
              <Input
                ...
                placeholder={
                  field.secret && isEdit ? t("payments.secretStored") : undefined
                }
              />
            </div>
          ))}
