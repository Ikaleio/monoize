...
use super::{
    ...
    CreateApiKeyWithLimitError, ModelRedirectRule, PlanChargeAllocation,
    RESERVED_INTERNAL_USER_PREFIX, RegisterUserError, RequestCaptureMode, RequestCaptureRetention,
    Session, UpdateApiKeyInput, User, UserBalance, UserRole, UserStore, canonicalize_group_ids,
...
pub(crate) fn plan_charge_meta(meta: &Value, allocation: &PlanChargeAllocation) -> Value {
    let mut object = meta.as_object().cloned().unwrap_or_default();
    object.insert(
        "charge_nano_usd".to_string(),
        Value::String(allocation.adjusted_charge_nano_usd.to_string()),
    );
    object.insert(
        "plan_covered_nano_usd".to_string(),
        Value::String(allocation.plan_covered_nano_usd.to_string()),
    );
    object.insert(
        "fallback_nano_usd".to_string(),
        Value::String(allocation.fallback_nano_usd.to_string()),
    );
    object.insert(
...
impl UserStore {
    ...
    pub async fn get_user_by_id(&self, id: &str) -> Result<Option<User>, String> {
        let row = self.db.read()
            .query_one(self.db.stmt(
                "SELECT id, username, password_hash, role, created_at, updated_at, last_login_at, enabled, balance_nano_usd, balance_unlimited, email, group_id FROM users WHERE id = $1",
                vec![id.into()],
            ))
            .await
            .map_err(|e| e.to_string())?;
    ...
    pub async fn get_user_by_username(&self, username: &str) -> Result<Option<User>, String> {
        let row = self.db.read()
            .query_one(self.db.stmt(
                "SELECT id, username, password_hash, role, created_at, updated_at, last_login_at, enabled, balance_nano_usd, balance_unlimited, email, group_id FROM users WHERE username = $1",
                vec![username.into()],
            ))
            .await
            .map_err(|e| e.to_string())?;
    ...
    pub async fn list_users(&self) -> Result<Vec<User>, String> {
        let rows = self.db.read()
            .query_all(self.db.stmt(
                "SELECT id, username, password_hash, role, created_at, updated_at, last_login_at, enabled, balance_nano_usd, balance_unlimited, email, group_id FROM users WHERE substr(lower(username), 1, 9) != '_monoize_' ORDER BY created_at DESC",
                vec![],
            ))
            .await
            .map_err(|e| e.to_string())?;
    ...
    async fn charge_user_balance_nano_inner(
        ...
        if user.unlimited {
            tx.commit()
                ...
                .map_err(|e| BillingError::new(BillingErrorKind::Internal, e.to_string()))?;
            return Ok(PlanChargeAllocation {
                adjusted_charge_nano_usd: amount_nano_usd,
                plan_covered_nano_usd: 0,
                fallback_nano_usd: 0,
                subscription_id: None,
                plan_id: None,
        ...
        let api_key_id = meta.get("api_key_id").and_then(Value::as_str);
        let billing_group_id = meta.get("billing_group_id").and_then(Value::as_str);
        let now = Utc::now();
        let allocation = self
        ...
        let next_balance = user
            .balance
            .checked_sub(allocation.fallback_nano_usd)
            .ok_or_else(|| {
        ...
        let ledger_meta = plan_charge_meta(meta, &allocation);

        self.insert_billing_ledger_tx(
            &tx,
            user_id,
            "request_charge",
            -allocation.fallback_nano_usd,
            Some(next_balance),
            &ledger_meta,
            &now_raw,
        )
