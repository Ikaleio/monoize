...
mod tests {
    ...
    use crate::users::{CreateGroupInput, UserRole};
    ...
    async fn store() -> UserStore {
        let db = DbPool::connect("sqlite::memory:").await.expect("database");
        {
            ...
            Migrator::up(&*write, None).await.expect("migrations");
        }
        let (tx, _) = tokio::sync::broadcast::channel(1);
        UserStore::new(db, tx).await.expect("store")
    }

    async fn group(store: &UserStore) -> String {
        store
    ...
    fn input(group_id: String) -> BillingPlanInput {
        BillingPlanInput {
            name: "Starter".to_string(),
            description: "Five-hour quota".to_string(),
            limit_5h_nano_usd: Some("1000".to_string()),
            limit_24h_nano_usd: Some("3000".to_string()),
            limit_7d_nano_usd: None,
            limit_30d_nano_usd: None,
            group_ids: vec![group_id],
            multiplier: "0.5".to_string(),
            listed: true,
            prices: vec![BillingPlanPriceInput {
                price_nano_usd: Some("500".to_string()),
                price_usd: None,
    ...
    #[tokio::test]
    async fn purchase_uses_prepaid_and_preserves_snapshot_after_plan_update() {
        let store = store().await;
        let group_id = group(&store).await;
        let plan = store
            .create_billing_plan(input(group_id.clone()))
            .await
            .expect("create")
            .expect("valid");
        let user = store
            .create_user("plan-user", "password123", UserRole::User, None)
            .await
            .expect("user");
        store
            .admin_adjust_user_balance(&user.id, Some("1000".to_string()), None, "admin")
            .await
            .expect("fund");
        let subscription = store
            .purchase_billing_plan(&user.id, &plan.id, &plan.prices[0].id)
            .await
            .expect("purchase")
            .expect("available");
        assert_eq!(subscription.plan_name, "Starter");
        assert_eq!(
            store
        ...
        let mut changed = input(group_id);
        changed.name = "Changed".to_string();
        store
            .update_billing_plan(&plan.id, changed)
            .await
            .expect("update")
            .expect("valid");
        assert_eq!(
    ...
    #[tokio::test]
    async fn allocation_applies_multiplier_and_sliding_limit_before_prepaid() {
        let store = store().await;
        let group_id = group(&store).await;
        let plan = store
            .create_billing_plan(input(group_id.clone()))
            ...
            .unwrap();
        let user = store
            .create_user("charge-user", "password123", UserRole::User, None)
            .await
            .unwrap();
        store
            .admin_adjust_user_balance(&user.id, Some("2000".to_string()), None, "admin")
            .await
            .unwrap();
        store
            .purchase_billing_plan(&user.id, &plan.id, &plan.prices[0].id)
