...
pub(crate) async fn emit_synthetic_chat_stream(
    ...
    let merged = urp::merged_output_items(&resp.outputs);
    let Item::Message { parts, .. } = merged else {
        unreachable!("merged assistant output must be a message")
    };
