...
mod tests {
    ...
    fn pay_amount(credit_usd: &str, rate: &str, scale: u32) -> String {
        let credit_nano = parse_positive_usd_to_nano(credit_usd).expect("credit parses");
        let rate = parse_positive_decimal(rate).expect("rate parses");
        let units = pay_minor_units(credit_nano, rate, scale).expect("pay amount computes");
        format_minor_units(units, scale)
    }

    /// Spec §15 T1: RC-U6 conversion vectors.
    #[test]
    fn conversion_matches_spec_vectors() {
        assert_eq!(pay_amount("10", "7.30", 2), "73.00");
        assert_eq!(pay_amount("10", "7.333333333", 2), "73.34");
        assert_eq!(pay_amount("10", "150", 0), "1500");
        // Ceiling on a sub-minor-unit remainder.
        assert_eq!(pay_amount("0.001", "7.30", 2), "0.01");
    }

    #[test]
    fn rate_format_is_strict() {
        for raw in [
            "",
            "0",
            "-1",
            ...
            assert!(parse_positive_decimal(raw).is_err(), "accepted {raw:?}");
        }
        assert_eq!(
            parse_positive_decimal("0.5").expect("parses"),
            PositiveDecimal {
                mantissa: 5,
                scale: 1
            }
    ...
    #[test]
    fn decimal_equality_normalizes_scale() {
        assert!(decimals_equal("73.00", "73.0"));
        assert!(decimals_equal("73", "73.000"));
        assert!(!decimals_equal("73.00", "73.01"));
        assert!(!decimals_equal("73", "no"));
    }
}
