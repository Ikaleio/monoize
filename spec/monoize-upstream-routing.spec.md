...
STRM-2a. Pre-output window. After an upstream streaming attempt returns HTTP `2xx`, or completes the WebSocket `response.create` send under WS2d, the attempt stays fallback-eligible until the stream decoder yields its first output event. An output event is every canonical stream event other than `ResponseStart`. Monoize MUST buffer `ResponseStart` and MUST NOT emit the downstream start envelope derived from it (for example Responses `response.created` and `response.in_progress`) before the window closes. When the window closes, Monoize MUST emit the buffered events before the first output event, in decoder order. The decoder idle timeout (`monoize_stream_idle_timeout_ms`) bounds the window.

STRM-2b. If the decoder yields a terminal `Error` event, or ends without an output event (EOF, `[DONE]`, idle timeout, decode failure, or WebSocket close), while the window is open, Monoize MUST end that attempt as an upstream attempt failure. Monoize MUST record one `tried_providers` entry for the attempt, MUST record the capture attempt dump with the error under `request-capture-dumps.spec.md`, and MUST apply the RTA-4, RTA-4a, and RTA-5 retry rules. The passive-failure class comes from the STRM-4 classification of the error code, error type, and synthesized HTTP status. Adapter failures whose code starts with `upstream_` have the Transient class. The synthesized status MUST NOT trigger an RTA-6c shared-origin blast. If no eligible attempt remains, RTA-8 applies and the downstream stream ends with the FP4e terminal error frame.

STRM-2c. A `ResponseDone` event that arrives as the first output event closes the window and commits the attempt. After the window closes, STRM-1 and STRM-4 govern the stream.
