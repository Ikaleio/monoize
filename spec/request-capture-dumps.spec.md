...
2. the request made more than one upstream attempt, including a retry against the same
   Channel or a fallback to another Channel or Provider;
3. the final request result contains no upstream `usage` object;
4. the final request result contains an upstream `usage` object whose
   `input_tokens + output_tokens == 0`.

A request that succeeds on its first and only upstream attempt with positive upstream
usage MUST NOT persist in this mode. A persisted dump MUST include all attempts retained
for that request in chronological order, subject to the RCD-C14 and RCD-D13 bounds.

RCD-C13. For RCD-C12, synthetic usage created only for estimated billing MUST NOT cause
condition 3 to become false. The abnormal-only decision MUST be based on
upstream-provided usage data before estimated billing fallback.
...
RCD-D2c. Dumps for Image API sub-requests MUST record `is_stream` equal to the parsed downstream `stream` flag (`image-api-proxy.spec.md` IG3/IE4): `false` for the non-streaming contract regardless of the internal upstream transport, `true` for the §5.5 streaming contract.
...
RCD-D4a. For multipart `POST /v1/images/edits`, `raw_input` MUST use the RCD-D16 multipart capture object. It MUST contain consumed text and accepted files in wire order. Text MUST retain its exact value before JSON coercion. Accepted files (`image`, `image[]`, `mask`) MUST retain their raw bytes. Ignored files MUST NOT appear. JSON edit requests MUST instead retain the parsed JSON body under RCD-D4, before reference conversion.
...
RCD-D10c. A stream-collected attempt uses the internal URP stream pipeline without capturing downstream SSE emissions. This includes synthetic terminal events decoded from upstream JSON for an Image API SSE response. Native SSE collection paths include: the non-stream collection path for `responses`, `openai_image`, and `openrouter_image` upstream attempts whose transformed request has `stream == true`, the Image API sub-request path whose attempts all target `responses` providers (`image-api-proxy.spec.md` IM15), and the Image API streaming sub-request path (`image-api-proxy.spec.md` IM3a/§5.5). For a stream-collected attempt whose decoded URP stream (before response-phase transforms) emitted a terminal `response_done` event, `reconstructed_urp_response` MUST be that event serialized in the RCD-D10a shape. It therefore reflects the upstream artifact before response transforms, unlike RCD-D10a, because a stream-collected attempt has no `downstream_response` to fill that role. For a stream-collected attempt, `downstream_response` and `downstream_sse_frames` MUST be null or absent: upstream SSE frames are not downstream emissions and are not captured, and §5.5 downstream image frames are derived artifacts that are likewise not captured.
