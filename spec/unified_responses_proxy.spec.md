...
WS2. Downstream WebSocket is a client transport. Request assembly, Channel selection, adaptation, retry, transform, billing, request-log, and Responses stream decoding MUST remain the existing pipeline. Continuation state MUST stay on the Monoize connection. Monoize MUST NOT send `previous_response_id` to an upstream WebSocket and MUST NOT keep an upstream WebSocket across turns.

WS2a. `POST /v1/responses` MUST set `prefer_upstream_websocket=false`. The Responses WebSocket handler MUST call the same Responses execution entry with `prefer_upstream_websocket=true`.

WS2b. When `prefer_upstream_websocket` is true, the selected attempt's effective API type is `responses`, and Channel `websocket_supported` is not `false`, Monoize MUST first attempt `GET {base_url}/v1/responses` as a WebSocket upgrade. The upgrade MUST send `Authorization`, `OpenAI-Beta: responses_websockets=2026-02-06`, Channel `extra_headers`, and session-affinity headers from CM-AFF. The handshake timeout MUST be the attempt `request_timeout_ms`. Egress MUST use Channel `proxy_url` when set, otherwise the process upstream proxy, otherwise a direct connection. HTTP and HTTPS proxies MUST use CONNECT. The CONNECT request-target and Host MUST bracket an IPv6 literal. Bytes after the CONNECT header terminator MUST remain on the tunneled stream. Upstream WebSocket message and frame limits MUST equal WS13 (`MONOIZE_RESPONSES_WS_MESSAGE_MAX_BYTES`, default 50 MiB).

WS2c. Handshake failure is TCP/TLS/CONNECT failure, a non-101 response, or upgrade timeout. On handshake failure Monoize MUST use HTTP SSE for that same attempt, MUST NOT increment the retry count, and MUST NOT record a passive failure. After status 101, send or decode failure MUST be treated as an ordinary upstream failure. Monoize MUST NOT fall back to HTTP after 101.

WS2d. After a successful upgrade, Monoize MUST send one text frame whose JSON is the already encoded Responses request object with `type` set to `response.create` and with `stream`, `background`, `generate`, `type` (before replacement), and `previous_response_id` removed. Monoize MUST read text frames until `response.completed`, `response.failed`, `response.incomplete`, `response.cancelled`, or `error`. Each JSON object MUST enter the same Responses stream decoder used for SSE JSON events. Monoize MUST then close that upstream WebSocket. Monoize MUST NOT multiplex with upstream `stream_id`.

WS2e. Channel `websocket_supported` is `boolean | null`. `null` means unknown. `true` means prefer WebSocket. `false` means HTTP only. Lazy memory writes MAY run only when the stored value is `null`. A primary MUST execute `UPDATE monoize_channels SET websocket_supported = $1 WHERE id = $2 AND websocket_supported IS NULL`. Status 101 MUST write `true` only when the stored value is `null`. Handshake failure MUST write `false` only when the stored value is `null`. The statement MUST NOT overwrite an explicit `true` or `false`. After handshake failure, an in-flight attempt whose effective memory is already `true` MUST keep `true` so a later same-Channel retry still prefers WebSocket; that failed handshake still uses HTTP SSE (WS2c). A successful write MUST increment `routing_config_revision`, and the in-flight attempt MUST adopt that new revision before health or affinity updates. A replica MUST NOT write the column. A replica MAY store the same boolean in a process-local map keyed by `channel_id` and MUST forget it on restart. Effective memory is the stored column when non-null, otherwise the replica overlay.

WS2f. HTTP POST/SSE clients, non-Responses upstreams, active health probes, and Channel liveness tests MUST use HTTP. They MUST NOT open an upstream WebSocket.

WS3. Each client message MUST be one UTF-8 JSON object in one WebSocket text message. Monoize MUST accept the Codex Responses WebSocket v1 beta header `OpenAI-Beta: responses_websockets=2026-02-04`, the Codex Responses WebSocket v2 beta header `OpenAI-Beta: responses_websockets=2026-02-06`, and an upgrade request without either header. The v1 beta header selects v1 terminal events. The v2 beta header takes precedence when both versions are supplied. Without a version header, `response.append` selects v1; `response.create` selects v2 unless v1 was already selected on the connection.

WS4. Monoize MUST process at most one response generation at a time per connection. The connection MAY receive multiple requests sequentially. Monoize MUST send every downstream Responses stream event as one JSON WebSocket text message. Monoize MUST NOT wrap the JSON in SSE `event:` or `data:` lines and MUST NOT send the SSE `[DONE]` sentinel over WebSocket. For v1, a successful `response.completed` event MUST be emitted as `response.done` after the bridge records its terminal output. V2 MUST retain `response.completed`. Failure and incomplete events MUST retain their original types.
...
WS7. After a successful terminal event (`response.done` for v1 or `response.completed` for v2), Monoize MUST retain, for that connection only and only within WS14 history limits:
...
S2. HTTP Responses requests default to `store=true`. Monoize MUST consume this control locally. Responses upstream requests MUST use `store=false`; other upstream families MUST omit it. Every generated HTTP Responses object MUST expose a new Monoize response id. A successful completed or incomplete response with `store=true` MUST retain its canonical input and output under that id, subject to S3a. Failed and cancelled responses MUST NOT create history entries.

S3. Monoize MUST resolve a non-null `previous_response_id` locally before routing or request transforms. The lookup scope MUST contain the authenticated tenant and API key identity. The next canonical input MUST contain the retained input, retained output, and current input in that order. Retained top-level `instructions` nodes MUST NOT carry into the next request. Current request controls MUST NOT inherit prior controls. Monoize MUST remove `previous_response_id` before upstream encoding and echo the client-supplied id in the downstream response. An unknown, expired, evicted, or differently scoped id MUST fail with HTTP 400, code `previous_response_not_found`, and param `previous_response_id`. A non-string id or a request containing both `conversation` and `previous_response_id` MUST fail with HTTP 400. `conversation` without `previous_response_id` remains native same-Responses passthrough and MUST be removed for other families.

S3a. History MUST be process-local and MUST expire 30 minutes after insertion. Restart MUST discard it. One retained history MUST contain at most 4096 nodes and 33554432 serialized bytes. The cache MUST contain at most 1024 entries and 134217728 serialized bytes. Insertion MUST evict the oldest entries until both cache limits hold. Oversized histories MUST NOT be retained. A stream MUST retain history before emitting its successful terminal response event. `store=false` MUST prevent HTTP history retention but MUST NOT disable connection-local WebSocket state. Historical canonical nodes MUST retain internal metadata; downstream JSON MUST NOT be the HTTP history representation.
...
RC4. Effort `none` requests disabled reasoning. An absent effort permits the upstream default. Monoize MUST NOT replace explicit `none` with an absent effort. When the selected upstream provider type is:

- `chat_completion`: Monoize MUST send every present normalized effort, including `none`, as `reasoning_effort`, except for the native reasoning-object case below. If the typed request preserves an explicit native `reasoning` object, that object is the single reasoning-control container: Monoize MUST preserve its non-`effort` members, MUST replace a colliding `reasoning.effort` with the normalized typed effort, and MUST omit the top-level `reasoning_effort` shorthand. OpenRouter `reasoning.effort` and `reasoning.max_tokens` are mutually exclusive. Therefore, when normalized typed effort is inserted into a native `reasoning` object, Monoize MUST remove a colliding `max_tokens`; when typed effort is absent, Monoize MAY preserve native `reasoning.max_tokens`. Monoize MUST NOT emit both equivalent effort controls in one request.
- `responses`: Monoize MUST send every present normalized effort, including `none`, as `reasoning: { "effort": <level> }`. Monoize MUST preserve other reasoning members such as `summary`.
- `messages`: Monoize MUST select the encoding based on the upstream model and any explicit downstream Messages controls:
  - If effort is `none` and no explicit Messages controls exist, Monoize MUST generate `thinking: { "type": "disabled" }`. Monoize MUST NOT generate `budget_tokens` or `output_config.effort`. Outbound validation under RC4f still applies.
  - For models that support adaptive thinking and effort other than `none`, a generated config MUST send `thinking: { "type": "adaptive" }`. If normalized reasoning effort is present and is not `none` or `minimal`, Monoize MUST also send `output_config: { "effort": <level> }`, transmitting `low`, `medium`, `high`, `xhigh`, and `max` as distinct values. If effort is absent, Monoize MUST omit generated `output_config`.
  ...
  - For non-adaptive models and effort other than `none`, Monoize MUST send `thinking: { "type": "enabled", "budget_tokens": N }`, where:
    - `minimal -> N=1024`
    - `low -> N=1024`
    - `medium -> N=4096`
    - `high -> N=16384`
...
PR4c.2a. When encoding an upstream `POST /v1/responses` request whose replayed `input[]` history contains tool-calling nodes, Monoize MUST preserve plaintext RawCoT reasoning items from open-source reasoning models. A reasoning item MAY be dropped only when a decoder or transform has explicitly marked it as downstream-only presentation text with `Reasoning.metadata.downstream_only = true`. The presence or absence of `encrypted_content` alone MUST NOT cause a raw `content[].reasoning_text` item to be dropped.
...
PR4c.5a. When applying PR4c.3 to a streaming reasoning delta, if the canonical URP stream event carries a non-empty `NodeDelta::Reasoning.metadata.item_id`, Monoize MUST store that value as the mz2 envelope `item_id`. Monoize MUST NOT emit an mz2 envelope with `item_id = null` when the stream event contains a non-empty reasoning item id.
...
PR4c.5c. Anthropic Messages `signature_delta.signature` values and legacy Chat scalar encrypted deltas are raw string fragments. When `reasoning_envelope_enabled = true`, Monoize MUST concatenate those raw fragments per canonical reasoning node before envelope construction. It MUST construct one mz2 envelope from the complete terminal value and MUST NOT concatenate two or more independently constructed mz2 envelopes. A structured Chat `reasoning.encrypted` detail is one complete ordered `reasoning_details[]` entry; Monoize MUST wrap its `data` value independently and MUST preserve its entry identity and sequence position. The canonical `Reasoning.encrypted` value MUST contain the envelope before response transforms run. Chat detail metadata MUST NOT retain a second data copy.
...
PR5d. A same-Responses decoder MUST preserve the non-empty upstream response `id` and integer `created_at` from `response.created` or `response.in_progress`. Before downstream encoding, the HTTP history stage MUST replace the response id under S2. Downstream start and terminal events MUST use the same Monoize id and preserve created_at. Unknown and optional fields present in the upstream start Responses object MUST survive downstream start-envelope reconstruction. Optional start-object fields absent upstream MUST remain absent except for the local history fields in S2/S3. If no upstream start Responses object exists before the first output event, Monoize MAY synthesize the start object and its identity.
...
- During that merge, a missing field or an empty string in one source MAY be filled from a non-empty value in another source. Except for the function-argument JSON-equality, function-argument done-snapshot, function-argument projected-field, message boundary-whitespace, terminal reasoning-summary, and terminal encrypted-content rules below, differing non-empty typed semantic values for the same slot MUST fail reconciliation.
- For two matched Function ToolCall nodes, differing non-empty arguments MUST be accepted when both parse as equal `serde_json::Value` values. Preserve the terminal argument string exactly. This rule permits JSON formatting, object-key order, and equivalent string escapes to differ. Different parsed values or invalid JSON MUST retain the existing conflict behavior unless a later function-argument rule in this list accepts the terminal string. Custom ToolCall input MUST retain exact string comparison except for the done-snapshot rule.
- A non-empty `arguments` string in `response.function_call_arguments.done`, or a non-empty `arguments` or `input` string on a matching `function_call` or `custom_tool_call` item in `response.output_item.done`, is the complete value for that tool-call slot. Monoize MUST replace the delta-accumulated argument string with that snapshot before comparing the slot with `response.completed.response.output[]`. An empty done-snapshot field MUST NOT erase a non-empty accumulated field.
- For two matched Function ToolCall nodes, if the accumulated argument string is non-empty and the terminal argument string parses as a JSON object, Monoize MUST accept the terminal argument string when the accumulated string equals the string value of key `input`, or equals the string value of key `patch`, `command`, or `content` when that value contains `Begin Patch`. Monoize MUST preserve the terminal argument string exactly and MUST NOT emit `responses_terminal_conflict`. This exception MUST NOT apply to Custom ToolCall input.
...
- If a decoded Messages `thinking` block contains non-empty summarized thinking without a signature or a recovered reasoning item id under PM5b, the decoder MUST set `Reasoning.metadata.downstream_only = true`. A Responses request encoder MUST omit a reasoning node with this marker because the summarized Messages presentation is not a Responses RawCoT replay item. A same-Messages encoder MUST ignore the marker and reconstruct the original thinking block. If PM5b recovers a non-empty reasoning item id, the decoder MUST NOT set this marker solely because the block uses the Messages shape.
...
- When the upstream Messages output contains a `redacted_thinking` block, Monoize MUST convert it into one URP `Reasoning` node with:
  ...
  - `metadata.redacted` set to `true` so that a later encode step targeting Messages can reconstruct the original block type.
...
PG4c. The Gemini encoder MUST initialize `generationConfig` from native passthrough configuration and overlay explicitly present typed fields. It MUST preserve unrelated native fields. Typed `stop` maps to `stopSequences`. Typed JSON object output maps to `responseMimeType = "application/json"`. Typed JSON schema output additionally maps to `responseJsonSchema`. Typed text output clears native response schemas and sets `responseMimeType = "text/plain"`. Typed reasoning controls replace recognized thinkingConfig fields under CTRL-1 through CTRL-7 in `urp-v2-flat-structure.spec.md`. Native decoding MUST retain only unknown thinkingConfig members. It MUST NOT approximate a native thinking budget with effort.

PG4d. A Gemini `functionCall.id`, when supplied, MUST remain the canonical `ToolCall.call_id`. A call without an id MUST receive a unique synthetic canonical call id. The encoder MUST omit synthetic ids from native calls and their matching native results. A `functionResponse` MUST preserve the matching supplied call id and the function name. Consecutive tool results MUST share one user Content. Native function-response objects MUST remain JSON objects during same-Gemini replay.
...
1. If `Reasoning.metadata.redacted == true` and `Reasoning.encrypted` is present, emit `{"type": "redacted_thinking", "data": <encrypted>}`.
...
PA1. A Chat message with an object-valued `configuration_update` MUST remain one complete ordered message, including empty content, its original role, and all non-internal fields. It MUST replay as a message only to Chat upstreams. Other protocol families MUST omit it.
